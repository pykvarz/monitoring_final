...
class FilterManager:
    ...
    def __init__(self, search_edit: QLineEdit, group_filter: QComboBox, 
        ...
        # Фильтр по статусу, установленный через карточки дашборда
        # (например "OFFLINE", "ONLINE"). None = показывать все.
        self._dashboard_status_filter: Optional[str] = None
    ...
    def apply_filters(self) -> None:
        ...
        for row in range(self._table_model.rowCount()):
            ...
            # Фильтр по статусу из дашборда (карточки Total/Online/Offline/…)
    ...
    def set_dashboard_status_filter(self, status: Optional[str]) -> None:
        """
        Установка фильтра по статусу из дашборда.

        Args:
            status: Код статуса ("ONLINE", "OFFLINE", "WAITING", "MAINTENANCE")
                    или None, чтобы снять фильтр.
        """
        self._dashboard_status_filter = status
        self.apply_filters()

    def reset_filters(self) -> None:
        """Сброс всех фильтров (включая фильтр дашборда)"""
        ...
        # Вызываем явно — сигналы заблокированы, чтобы не было двойного вызова
        self.apply_filters()
